...
// 이미지는 안정적 로드용 플레이스홀더(Lorem Picsum / pravatar)다 — 실제 사진 아님.
 ...
 */
export async function mockFetchPlaces() {
  await delay(550)
  // 상세용 사진 몇 장 부여
  return PLACES.map((p) => ({
    ...p,
    photos: [p.thumbnailUrl, photo(`${p.id}-b`), photo(`${p.id}-c`)].filter(Boolean),
  }))
}
...
export async function mockFetchPlace(id) {
  ...
  return { ...found, photos: [found.thumbnailUrl, photo(`${id}-b`), photo(`${id}-c`)].filter(Boolean) }
}
