...
**Don't**
- **컴포넌트 루트 엘리먼트에 일반적인 클래스명(`.login`, `.card`, `.list`, `.header` 등) 금지.** Vue `scoped` 스타일은 자식 컴포넌트의 **루트 엘리먼트에 부모의 scope id까지** 붙인다 → 부모와 자식이 같은 클래스명을 쓰면 부모의 `.x` / `.x:hover` 규칙이 자식 루트로 새어 깨진다. 루트 클래스는 **기능 접두사/BEM**으로 고유하게(예: `.login-page`, `.appbar__login`, `.place-list`).
